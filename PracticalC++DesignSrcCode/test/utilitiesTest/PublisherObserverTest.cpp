...
ConcreteObserver::ConcreteObserver(string_view name)
: Observer{name}
, notified_{false}
, state_{""}
{
...
void ConcreteObserver::notifyImpl(const std::any& data)
{
    notified_ = true;
    if( data.has_value() )
    {
        auto p = std::any_cast<string>(data);
        state_ = p;
    }
...
void PublisherObserverTest::testGetState()
{
    ...
    pt_->raise( "singleEvent", string{"sample state"});
