...
public:
    ...
    const vector<pdCalc::StackErrorData::ErrorConditions>& errors() const { return errors_; }
    void notifyImpl(const std::any&);

private:
    vector<string> messages_;
    vector<pdCalc::StackErrorData::ErrorConditions> errors_;
};
...
{
    auto p = std::any_cast<pdCalc::StackErrorData>(data);
...
void StackTest::testErrors()
{
    ...
    const string emptyMsg = pdCalc::StackErrorData::Message(pdCalc::StackErrorData::ErrorConditions::Empty);
    const string swapMsg = pdCalc::StackErrorData::Message(pdCalc::StackErrorData::ErrorConditions::TooFewArguments);
    ...
    const vector<pdCalc::StackErrorData::ErrorConditions> errors { raw->errors() };
    QVERIFY(errors.size() == 2);
    QCOMPARE(errors[0], pdCalc::StackErrorData::ErrorConditions::Empty);
    QCOMPARE(errors[1], pdCalc::StackErrorData::ErrorConditions::TooFewArguments);
