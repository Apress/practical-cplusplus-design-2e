...
void GuiModel::GuiModelImpl::onCharacterEntered(char c)
{
    ...
    if(state_.curInput.find('e') != string::npos && c == '.') return;

    // bug in QDoubleValidator that lets e be the first letter, so self validate
    if(state_.curInput.empty() && c == 'e') return;

    if(auto state = validate(state_.curInput + c); state != QValidator::Invalid)
    {
...
void GuiModel::GuiModelImpl::onBackspace()
{
    ...
    {
        state_.curInput.erase(state_.curInput.size() - 1, 1);
...
void GuiModel::GuiModelImpl::handlePlusMinus()
{
    ...
    if(auto pos = state_.curInput.find('e'); pos == string::npos)
    {
    ...
    {
        if(auto [left, right] = split(state_.curInput, pos); state_.curInput.size() > pos && state_.curInput[pos + 1] == '-')
        {
            state_.curInput = left + right.substr(1, right.length() - 1);
        }
        else
        {
            state_.curInput = left + '-' + right;
        }
    }

    if(auto state = validate(state_.curInput); state != QValidator::Invalid)
    {
        state_.curInputValidity = state;
        emit parent_.modelChanged();
    }
